...
The wiki has explicit "Controls:" prose for many drive types, and
similar prose for some phaser types. **HW-033 closed 2026-04-25**:
`scripts/extract-lineage.ts` now extracts these into a `controls`
field per lineage record. Reverb / delay / compressor / amp / chorus
/ flanger / wah wikis don't list per-type knobs (they describe
algorithms / models, not original-pedal control sets), so wiki-
derived coverage there is empty. **31 drive types + 1 phaser type**
now have wiki-derived knob lists in `src/knowledge/{block}-
lineage.json`, surfaced as a per-type table in **`docs/TYPE-KNOBS-
WIKI.md`** (auto-generated by `npm run build-type-knobs`). When a
hardware capture lands and disagrees with the wiki row, prefer the
capture — see this file's per-block tables below for captured
truth.
